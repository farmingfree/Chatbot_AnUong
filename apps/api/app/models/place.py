...
class Place(Base):
    ...
    google_place_id: Mapped[str | None] = mapped_column(String(100), unique=True, index=True)
    google_cid: Mapped[str | None] = mapped_column(String(100), unique=True, index=True)
    ...
    validation_flags = mapped_column(JSON, nullable=True)
    ...
    source_data = mapped_column(JSON, nullable=True)
